...
def train(run_folder: str):
    ...
    TRAIN_CMD = "accelerate launch -m axolotl.cli.train ./finalconfig.yml"
...
def merge(run_folder: str):
    ...
    with open(f"{run_folder}/finalconfig.yml") as config:
    ...
    MERGE_CMD = f"accelerate launch -m axolotl.cli.merge_lora ./finalconfig.yml --lora_model_dir='{MERGE_SRC}' --load_in_8bit=False --load_in_4bit=False --flash_attention=False"
...
def launch(config_raw: str, data_raw: str):
    ...
    with (
        open(f"{run_folder}/finalconfig.yml", "w") as config_file,
...
def main(config: str = "finalconfig.yml", dataset: str = "my_data.jsonl"):
    # Read finalconfig.yml and my_data.jsonl and pass them to the new function.
